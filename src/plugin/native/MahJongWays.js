...
//   }
//   async start () {
//     let res = ref.allocCString(JSON.stringify(this.obj))

//     this.dll.RunTest(res)

//     return JSON.parse(ref.readCString(res))
//   }
// }

const dllPath = 'resources/MahJongWays/MahJongWays.dll'
...
export function calResult (
  ...
  try {
    ...
    if (!back) {
      const dll = ffi.Library(path.resolve(dllPath), {
        RunTest: ['void', ['pointer']] //下注金額,次數,回傳結果
      })

      // let rawBuf=ref.allocCString(JSON.stringify(input))
      // console.log(`buffer raw length->${rawBuf.length}`)
      //var res = Buffer.from(rawBuf,0,1348)
      var res = Buffer.alloc(bufferSize)
      res.fill(0)
      res.write(JSON.stringify(input),0,'utf-8')

      //console.log(`buffer new length->${res.length}`)
    ...
    let runResult = function () {
      ...
      let dll = ffi.Library(path.resolve(dllPath), {
        RunTest: ['void', ['pointer']] //下注金額,次數,回傳結果
      })
