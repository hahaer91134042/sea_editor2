...
//第一關
let round = computed({
    get: () => lua.value?.levels[0] ?? null
})

let free_round = computed({
    get: () => free.value?.levels[0] ?? null
})
...
let rolls = reactive({
    ...
    array: computed({
        get: function () {
            ...
            let arr = free_round.value?.rolls.let(o => {
    ...
    item: {
        ...
        copy(type) {
            ...
            let sList = free_round.value.rolls.rolls[index]
            ...
            switch (type) {
                case 1: {//複製-列
                    ...
                    for (let i = si; i < ei; i++) {
                        free_round.value.rolls.rolls[i] = sList.map(o=>o.copy())
                    }
                    break;
                }
                default: {//全複製

                    for (let i = 0; i < free_round.value.rolls.rolls.length; i++) {
                        free_round.value.rolls.rolls[i] = sList.map(o=>o.copy())
                    }
                    break;
                }
...
let uc = reactive({
    w: computed({ get: () => 250 }),
    total: computed({
        get: () => free_round.value?.uc.total ?? 0,
        set: (v) => {
            if (free_round.value?.uc)
                free_round.value.uc.total = v
        }
    }),
    items: computed({
        get: function () {
            return free_round.value?.uc.let(u => {
                let row = rolls.rowNum
                let col = rolls.colNum
                ...
                for (let x = 0; x < col; x++) {
                    ...
                    for (let y = 0; y < row; y++) {
                        ...
                        let index = x * row + y
                        let u = free_round.value?.uc.rolls[index]
    ...
    findUcTarget: function (order) {
        let a = free_round.value?.uc.rolls ?? []
        let b = []
    ...
    removeItem(order, name, index) {
        let items = free_round.value?.uc.rolls.first(o => o.order == order)
    ...
    onChangeOrder() {
        ...
        free_round.value.uc.order = _.orderBy(free_round.value.uc.rolls, 'order', 'asc')
            .let(l1 => {
                //console.log(`lua uc orderBy result->${l1.jsonString()}`)
                let order = []
                l1.forEach(l => {
                    order.push(_.findIndex(free_round.value.uc.rolls, (o) =>
                        (o.order == l.order && o.uc == l.uc)) + 1)
                })
                //console.log(`lua uc new order->${order.jsonString()}`)
                return order
...
let kickout = reactive({
    ...
    items: computed({
        get: function () {
            if (!free_round.value)
                return []
            //先multiple 在quota
            let arr = tryObjToArray(free_round.value.kick_out.multiple)
                .map(a => a.copy().also(c => c.type = KO.Multiple))
                .concat(tryObjToArray(free_round.value.kick_out.quota)
    ...
    itemChange(index, value) {
        ...
        switch (value.type) {
            case KO.Multiple: {
                ...
                free_round.value.kick_out.multiple[index].copy(value)
                break;
            }
            case KO.Quota: {
                index = index - tryObjToArray(free_round.value.kick_out.multiple).length
                //console.log(`kickout item change Quota index->${index}`)
                free_round.value.kick_out.quota[index].copy(value)
                break;
            }
        }
    },
    total: computed({
        get: function () {
            if (!free_round.value)
                return 0
            return free_round.value.kick_out.total
        },
        set: function (v) {
            free_round.value.kick_out.total = v
        }
    }),
    onAdd() {
    ...
    onDelete(v) {
        ...
        switch (v.type) {
            case KO.Multiple: {
                let i = tryObjToArray(free_round.value.kick_out.multiple)
                    .findIndex(a => a.key_val == v.key_val && a.weight == v.weight)
                //console.log(`kickout multiple delete index->${i}`)
                if (i > -1)
                    free_round.value.kick_out.multiple.remove(i)
                break;
            }

            case KO.Quota: {
                let ii = tryObjToArray(free_round.value.kick_out.quota)
                    .findIndex(a => a.key_val == v.key_val && a.weight == v.weight)
                //console.log(`kickout quota delete index->${ii}`)
                if (ii > -1)
                    free_round.value.kick_out.quota.remove(ii)
                break;
            }
    ...
    addItem(v) {
        ...
        if (v == KO.Multiple) {
            free_round.value.kick_out.multiple = tryObjToArray(free_round.value.kick_out.multiple)
                .also(a => a.push(item))

        } else if (v == KO.Quota) {
            free_round.value.kick_out.quota = tryObjToArray(free_round.value.kick_out.quota)
                .also(a => a.push(item))
        }
...
function selectRollItemList() {//每個格子內部的array
    return rolls.item.now?.let(n => {
        ...
        let a = free_round.value?.rolls.rolls.slice(si, ei) ?? []
