    ...
    <div class="container mb-5" style="padding-top:10px;">
        ...
        <div class="border border-2 border-black " style="width:100%;height:470px; overflow-x: auto;">
            <div style="width: 150px;background-color: white;">
                {{ Lang.tc.roller }}({{ Lang.tc.mainGame }})
            </div>
...
import { inject, ref, reactive, toRefs, watch, computed } from 'vue'
...
let roll = reactive({
    ...
    slotCol: computed({
        get: function () {
            ...
            return lua.value.roll_slots.let(slots => {
                let d = {}
...
let gold = reactive({
    items: computed({
        get: function () {
            if (!lua.value)
                return []
            return tryObjToArray(lua.value.Gold)
        }
    })
})
...
watch(lua, (_) => {
    ...
    if (_) {
        lua.value.roll_slots.forEach(item => {
            watch(item.roll_slot,
                (now) => {
                    //console.log(`mainGame lua watch slot->${now.jsonString()}`)
